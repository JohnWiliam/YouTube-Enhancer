...
// @version      2.2.0
// @description  Reduz uso de CPU (Smart Mode), personaliza layout, remove Shorts, elimina blur e adiciona relógio.
...
(function() {
    ...
    const SmartCpuTamer = {
        ...
        state: { hidden: false, playing: false, audible: false, pip: false, networkBusy: false, throttlingLevel: 0 },
        handlers: { visibility: null, play: null, pause: null, ended: null, volumechange: null, enterpip: null, leavepip: null, pageData: null },
        mainMediaElement: null, mediaStatePoller: null, rafFallbackTimers: new Map(), rafFallbackId: 0,
        gracePeriodTimer: null, networkBusyTimer: null, GRACE_PERIOD_MS: 20000,

        init() {
            if (this.initialized) return;
            this.originals = {
                setInterval: targetWindow.setInterval,
        ...
        cleanup() {
            if (!this.initialized) return;
            const applyOverride = (name, original) => { try { targetWindow[name] = original; } catch(e) {} };
            ...
            Object.entries(this.handlers).forEach(([name, fn]) => {
                if (!fn) return;
                const useCapture = ['play', 'pause', 'ended', 'volumechange', 'enterpip', 'leavepip'].includes(name);
                document.removeEventListener(name === 'pageData' ? 'yt-page-data-updated' : name, fn, useCapture);
            });
            this.handlers = { visibility: null, play: null, pause: null, ended: null, volumechange: null, enterpip: null, leavepip: null, pageData: null };
            if (this.gracePeriodTimer) clearTimeout(this.gracePeriodTimer);
            if (this.networkBusyTimer) clearTimeout(this.networkBusyTimer);
        ...
        resolveMainMediaElement(force = false) {
            ...
            this.mainMediaElement = Utils.DOMCache.get('#movie_player video.html5-main-video', true) || Utils.DOMCache.get('.html5-video-player video', true) || null;
            return this.mainMediaElement;
        },
        isMainPlayerMediaEventTarget(target) { return target instanceof HTMLMediaElement && target === this.resolveMainMediaElement(); },
        refreshPlayingFromMainMedia() {
            ...
            this.state.audible = !!(media && !media.muted && media.volume > 0 && !media.paused);
            this.state.pip = !!(document.pictureInPictureElement && media && document.pictureInPictureElement === media);
        },
        flagNetworkBusy(ms = 4000) {
            this.state.networkBusy = true;
            if (this.networkBusyTimer) clearTimeout(this.networkBusyTimer);
            this.networkBusyTimer = setTimeout(() => { this.state.networkBusy = false; this.updateState(); }, ms);
        },
        bindEvents() {
            this.handlers.visibility = () => {
                ...
                if (this.state.hidden) {
                    ...
                    this.state.networkBusy = false;
                }
                this.updateState();
            };
            document.addEventListener('visibilitychange', this.handlers.visibility);
            ['play', 'pause', 'ended', 'volumechange', 'enterpip', 'leavepip'].forEach((eventName) => {
                this.handlers[eventName] = (e) => {
                    if (!this.isMainPlayerMediaEventTarget(e.target)) return;
                    this.refreshPlayingFromMainMedia();
                    if (eventName === 'play' || eventName === 'enterpip') this.flagNetworkBusy(5000);
                    this.updateState();
                };
                document.addEventListener(eventName, this.handlers[eventName], true);
            });
            this.handlers.pageData = () => { this.resolveMainMediaElement(true); this.flagNetworkBusy(6000); this.updateState(); };
            document.addEventListener('yt-page-data-updated', this.handlers.pageData);
            this.mediaStatePoller = setInterval(() => {
                const prev = `${this.state.playing}|${this.state.audible}|${this.state.pip}`;
                this.refreshPlayingFromMainMedia();
                const next = `${this.state.playing}|${this.state.audible}|${this.state.pip}`;
                if (prev !== next) this.updateState();
            }, 2000);
            this.state.hidden = document.visibilityState === 'hidden';
            this.refreshPlayingFromMainMedia();
        },
        updateState(forceOptimization = false) {
            ...
            if (!this.state.hidden || graceActive || this.state.pip || this.state.networkBusy) this.state.throttlingLevel = 0;
            else if (this.state.playing && this.state.audible) this.state.throttlingLevel = 1;
            else if (this.state.playing && !this.state.audible) this.state.throttlingLevel = 2;
            else this.state.throttlingLevel = 3;
        },
        overrideTimers() {
            const self = this;
            ...
            targetWindow.setInterval = function(callback, delay, ...args) {
                let d = norm(delay);
                if (self.state.throttlingLevel >= 3) d = Math.max(d, 12000);
                else if (self.state.throttlingLevel === 2) d = Math.max(d, 4000);
            ...
            targetWindow.setTimeout = function(callback, delay, ...args) {
                let d = norm(delay);
                if (self.state.throttlingLevel >= 3) d = Math.max(d, 3500);
                else if (self.state.throttlingLevel === 2) d = Math.max(d, 1200);
            ...
            targetWindow.requestAnimationFrame = function(callback) {
                if (self.state.throttlingLevel > 0) {
                    ...
                    const d = self.state.throttlingLevel >= 3 ? 1500 : self.state.throttlingLevel === 2 ? 350 : 66;
