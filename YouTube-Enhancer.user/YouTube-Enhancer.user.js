...
// @version      2.3.2
// @description  Personaliza o layout, remove Shorts e seções indesejadas, elimina blur e adiciona relógio.
...
(function() {
    ...
    const Utils = {
        ...
        sanitizeConfig(config, defaults) {
            ...
            for (const feature of Object.keys(defaults.FEATURES)) {
                safe.FEATURES[feature] = this.toBoolean(config?.FEATURES?.[feature], defaults.FEATURES[feature]);
            }
        ...
        pt: {
            modal: {
                ...
                features: {
                    ...
                    mostRelevant: { title: 'Remover (Mais Relevantes)', description: "Remove o elemento 'Mais relevantes' da página de inscrições." },
                    ...
                    language: { title: 'Idioma da Interface', description: 'Troca textos entre PT e EN' }
                },
                clockStyle: { textColor: 'Cor do Texto', backgroundColor: 'Cor do Fundo', backgroundOpacity: 'Opacidade Fundo', fontSize: 'Tamanho Fonte (px)', margin: 'Margem (px)', borderRadius: 'Arredondamento (px)' },
        ...
        en: {
            modal: {
                ...
                features: {
                    ...
                    videosPerRow: 'Videos per row', videosPerRowHint: 'Defines videos in each row',
                    shorts: { title: 'Remove Shorts', description: 'Cleans Shorts from UI' },
                    mostRelevant: { title: 'Remove (Most Relevant)', description: "Removes the 'Most relevant' element from the Subscriptions page." },
                    clock: { title: 'Floating Clock', description: 'Shows time over the video' },
                    rtx: { title: 'RTX Mode (no blur)', description: 'Turns translucent backgrounds transparent' },
                    language: { title: 'Interface Language', description: 'Switch texts between EN and PT' }
                },
    ...
    const ConfigManager = {
        ...
        defaults: {
            ...
            FEATURES: { LAYOUT_ENHANCEMENT: true, SHORTS_REMOVAL: true, MOST_RELEVANT_REMOVAL: false, FULLSCREEN_CLOCK: true, RTX_VISUAL_MODE: true },
        ...
        load() {
            try {
                ...
                const sanitized = Utils.sanitizeConfig(Utils.migrateConfig(saved, this.CONFIG_VERSION) || {}, this.defaults);
                if (JSON.stringify(saved) !== JSON.stringify(sanitized)) GM_setValue(this.STORAGE_KEY, sanitized);
                return sanitized;
    ...
    const UIManager = {
        ...
        async openSettings(onSave) {
            ...
            optionsList.append(
                ...
                createToggle('cfg-most-relevant', t('modal.features.mostRelevant.title', config.LANGUAGE), t('modal.features.mostRelevant.description', config.LANGUAGE), config.FEATURES.MOST_RELEVANT_REMOVAL),
            ...
            modal.setAttribute('lang', config.LANGUAGE === 'pt' ? 'pt-BR' : 'en');
            ...
            const getNewConfig = () => Utils.sanitizeConfig({
                ...
                FEATURES: {
                    ...
                    MOST_RELEVANT_REMOVAL: document.getElementById('cfg-most-relevant').checked,
    ...
    // =======================================================
    // MOST RELEVANT SHELF MANAGER
    // =======================================================
    const MostRelevantManager = {
        observer: null, listenersCleanup: [], hiddenElements: new Set(), enabled: false,
        debouncedPrune: Utils.debounce(function() { if (this.enabled) this.prune(); }, 250),
        init(config) {
        ...
        updateConfig(config) {
            const shouldEnable = Boolean(config?.FEATURES?.MOST_RELEVANT_REMOVAL);
            if (shouldEnable === this.enabled) return;
            this.enabled = shouldEnable;
            if (this.enabled) this.start(); else this.stop();
        },
        ...
        prune() {
            ...
            document.querySelectorAll('ytd-rich-shelf-renderer #dismissible.style-scope.ytd-rich-shelf-renderer').forEach((dismissible) => {
                const section = dismissible.closest('ytd-rich-section-renderer.ytd-rich-grid-renderer.style-scope');
                this.markHidden(section || dismissible);
            });
        },
        cleanup() { this.stop(); }
    };
    ...
    const ClockManager = {
        ...
        setupObserver() {
            ...
            this.observer = new MutationObserver(Utils.debounce(() => { this.adjustPosition(); this.updateVisibility(); }, 100));
            this.observer.observe(this.playerElement, { attributes: true, attributeFilter: ['class', 'style', 'aria-hidden'], childList: true, subtree: true });
        },
        isPlayerMenuOpen() {
            if (!this.playerElement) return false;
            return Array.from(this.playerElement.querySelectorAll('.ytp-settings-menu, .ytp-popup.ytp-contextmenu')).some((menu) => {
                const style = window.getComputedStyle(menu);
                return menu.getAttribute('aria-hidden') !== 'true' && style.display !== 'none' && style.visibility !== 'hidden' && style.opacity !== '0';
            });
        },
        updateVisibility() {
            if (!this.clockElement) return;
            const shouldShow = Boolean(document.fullscreenElement) && !this.isPlayerMenuOpen();
    ...
    const EnhancerCore = {
        init() {
            try {
                ...
                try { MostRelevantManager.init(config); } catch (e) { console.error('[YT Enhancer] MostRelevantManager init failed:', e); }
                ...
                Utils.safeAddEventListener(window, 'beforeunload', () => {
                    ClockManager.cleanup(); ShortsManager.cleanup(); MostRelevantManager.cleanup(); Utils.DOMCache.refresh();
                });
