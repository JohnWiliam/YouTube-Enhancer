...
(function() {
    ...
    const SmartCpuTamer = {
        ...
        state: { hidden: false, playing: false, throttlingLevel: 0, connectedPwaSession: false, lastUserInteraction: 0 },
        handlers: { visibility: null, play: null, pause: null, ended: null, activity: null },
        ...
        cleanup() {
            ...
            if (this.handlers.activity) ['pointerdown', 'keydown', 'touchstart'].forEach((evt) => document.removeEventListener(evt, this.handlers.activity, true));
            this.handlers = { visibility: null, play: null, pause: null, ended: null, activity: null };
        ...
        resolvePwaCompatibilityHints() {
            const isStandalone = targetWindow.matchMedia?.('(display-mode: standalone)')?.matches || navigator.standalone === true;
            const isPictureInPicture = !!document.pictureInPictureElement;
            const activeMediaSession = !!(navigator.mediaSession && this.state.playing);
            const hasRecentInteraction = Date.now() - this.state.lastUserInteraction < 15000;
            this.state.connectedPwaSession = Boolean(isStandalone || isPictureInPicture || activeMediaSession || hasRecentInteraction);
            return this.state.connectedPwaSession;
        },

        trackActivitySignals() {
            const bump = () => { this.state.lastUserInteraction = Date.now(); if (this.state.hidden) this.updateState(); };
            ['pointerdown', 'keydown', 'touchstart'].forEach((evt) => document.addEventListener(evt, bump, { passive: true, capture: true }));
            this.handlers.activity = bump;
        },

        updateState(forceOptimization = false) {
            this.refreshPlayingFromMainMedia();
            const pwaSensitive = this.resolvePwaCompatibilityHints();
            ...
            else if (this.state.playing || pwaSensitive) this.state.throttlingLevel = 1;
        ...
        overrideTimers() {
            ...
            applyOverride('setInterval', function(callback, delay, ...args) {
                let d = norm(delay);
                if (self.state.throttlingLevel === 2) d = Math.max(d, 7000);
                else if (self.state.throttlingLevel === 1) d = Math.max(d, self.state.playing ? 850 : 1200);
            ...
            applyOverride('setTimeout', function(callback, delay, ...args) {
                let d = norm(delay);
                if (self.state.throttlingLevel === 2) d = Math.max(d, 3000);
                else if (self.state.throttlingLevel === 1) d = Math.max(d, self.state.playing ? 180 : 450);
            ...
            applyOverride('requestAnimationFrame', function(callback) {
                if (self.state.throttlingLevel > 0) {
                    ...
                    const d = self.state.throttlingLevel === 1 ? 40 : 1200;
