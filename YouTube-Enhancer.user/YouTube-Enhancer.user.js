...
(function() {
    ...
    const SmartCpuTamer = {
        ...
        state: { hidden: false, playing: false, throttlingLevel: 0, inPictureInPicture: false, interactionHoldUntil: 0 },
        handlers: { visibility: null, play: null, pause: null, ended: null, userActivity: null, pipEnter: null, pipLeave: null, pagehide: null, pageshow: null },
        ...
        cleanup() {
            ...
            if (this.handlers.userActivity) ['pointerdown', 'keydown', 'touchstart'].forEach((evt) => document.removeEventListener(evt, this.handlers.userActivity, true));
            if (this.handlers.pipEnter) document.removeEventListener('enterpictureinpicture', this.handlers.pipEnter, true);
            if (this.handlers.pipLeave) document.removeEventListener('leavepictureinpicture', this.handlers.pipLeave, true);
            if (this.handlers.pagehide) window.removeEventListener('pagehide', this.handlers.pagehide);
            if (this.handlers.pageshow) window.removeEventListener('pageshow', this.handlers.pageshow);
            this.handlers = { visibility: null, play: null, pause: null, ended: null, userActivity: null, pipEnter: null, pipLeave: null, pagehide: null, pageshow: null };
        ...
        bindEvents() {
            ...
            this.handlers.userActivity = () => { this.state.interactionHoldUntil = Date.now() + this.INTERACTION_HOLD_MS; this.updateState(); };
            ['pointerdown', 'keydown', 'touchstart'].forEach((evt) => document.addEventListener(evt, this.handlers.userActivity, true));
            this.handlers.pipEnter = () => { this.state.inPictureInPicture = true; this.updateState(); };
            this.handlers.pipLeave = () => { this.state.inPictureInPicture = false; this.updateState(); };
            document.addEventListener('enterpictureinpicture', this.handlers.pipEnter, true);
            document.addEventListener('leavepictureinpicture', this.handlers.pipLeave, true);
            this.handlers.pagehide = (e) => { if (e.persisted) this.updateState(true); };
            this.handlers.pageshow = () => this.updateState();
            ...
            this.state.inPictureInPicture = !!document.pictureInPictureElement;
            this.refreshPlayingFromMainMedia();
            this.state.interactionHoldUntil = Date.now() + this.INTERACTION_HOLD_MS;
        },

        updateState(forceOptimization = false) {
            this.refreshPlayingFromMainMedia();
            const interactiveSessionActive = this.state.interactionHoldUntil > Date.now();
            const pipActive = this.state.inPictureInPicture || !!document.pictureInPictureElement;
            const mediaSessionActive = navigator?.mediaSession?.playbackState === 'playing';
            const pwaVisible = window.matchMedia?.('(display-mode: standalone)').matches && !this.state.hidden;
            const graceActive = this.state.hidden && !forceOptimization && this.gracePeriodTimer;
            if (!this.state.hidden || graceActive || pipActive || mediaSessionActive || pwaVisible || interactiveSessionActive) this.state.throttlingLevel = 0; 
        ...
        overrideTimers() {
            ...
            applyOverride('setTimeout', function(callback, delay, ...args) {
                let d = norm(delay);
                if (self.state.throttlingLevel === 2) d = Math.max(d, 1500);
