...
3. **🧹 Remove “Most Relevant”**
   ...
   - Reacts to YouTube's SPA navigation and restores the shelf immediately when disabled.
...
- Early dynamic CSS injection for progressive rendering
- Scoped mutation processing for YouTube SPA updates
- Frame-coalesced player reactions, DOM caching, and lifecycle cleanup
...
3. **🧹 Remover “Mais relevantes”**
   ...
   - Acompanha a navegação SPA do YouTube e restaura a prateleira imediatamente quando a opção é desativada.
...
- Injeção antecipada de CSS para a renderização progressiva
- Processamento de mutações com escopo para atualizações da SPA do YouTube
- Reações do player agrupadas por quadro, cache de DOM e limpeza de ciclo de vida
